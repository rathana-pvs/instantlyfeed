...
echo -e "${GREEN}🚀 Starting InstantlyFeed Automated VPS Setup...${NC}"
...
if [ ! -f .env ]; then
    ...
    echo -e "${GREEN}scp /home/rathana/Desktop/instantlyfeed/.env root@YOUR_VPS_IP:$(pwd)/.env${NC}"
    exit 1
fi
...
if [ -n "$VOLUME_NAME" ] && docker run --rm -v "$VOLUME_NAME":/etc/letsencrypt alpine ls /etc/letsencrypt/live/instantlyfeed.com/fullchain.pem >/dev/null 2>&1; then
...
echo -e "${GREEN}✅ Setup complete! InstantlyFeed is now running at https://instantlyfeed.com${NC}"
