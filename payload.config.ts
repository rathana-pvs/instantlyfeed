...
export default buildConfig({
  ...
  db: postgresAdapter({
    pool: {
      connectionString: process.env.DATABASE_URI || 'postgresql://instantlyfeed:instantlyfeed123@localhost:5435/instantlyfeed',
    },
  }),
  plugins: [
    seoPlugin({
      ...
      generateTitle: ({ doc }: { doc: any }) => doc?.title ? `${doc.title} — InstantlyFeed` : 'InstantlyFeed',
